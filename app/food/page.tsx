...
import { getUserFoodItems, deleteFoodItem, createShare, type FoodItem as DBFoodItem } from '../lib/database'
...
export default function YourFoodPage() {
    ...
    const tabs = ['All Items', 'Expiring Soon', 'Fresh', 'Fridge', 'Pantry', 'Freezer']
    ...
    const loadUserAndItems = async () => {
        try {
            ...
            setUserId(user.id)
            const dbItems = await getUserFoodItems(user.id)
    ...
    const handleShareToCommunity = async () => {
        if (!userId) return
        try {
            await Promise.all(selectedItems.map(id => createShare(id, userId)))
            setSelectedItems([])
            ...
            <div className='ml-64'>
                <header className='bg-[#443104] px-8 py-6 flex items-center justify-end gap-4 fixed top-0 left-64 right-0 z-10'>
                    <button className='relative'>
                        <Image src='/Notifications.svg' alt='Notifications' width={28} height={28} />
                ...
                <main className='p-8 space-y-6 mt-[88px]'>
                    <h1 className='text-4xl font-bold text-[#443104]'>Your Food</h1>

                    {loading ? (
                        <div className='text-center py-12 text-[#443104]'>Loading...</div>
                    ) : (
                        <>
                    ...
                    <div className='bg-[#443104] rounded-2xl p-6 text-white'>
                        <div className='grid grid-cols-2 gap-6 text-center'>
                            ...
                            <div>
                                <p className='text-3xl font-bold'>{items.filter(i => getDaysUntilExpiry(i.expiry) <= 7).length}</p>
                                <p>Expiring Soon</p>
                            </div>
                        </div>
                    </div>
